...
4. Reprioritize / reorder: sequence `Order` again, adjust `Phasing` for work not yet built; foundations stay first; work dropped from scope → `dropped`. If the project's risk profile has clearly shifted (e.g. payments or auth were added), re-recommend the `**Workflow:**` default via the plan Step 5b panel; otherwise leave it.
