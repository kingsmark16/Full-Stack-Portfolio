...
- **Cross check the drafted spec** (its primary job is decision completeness: finding values an action must produce whose source the spec never names, and decisions the builder would otherwise invent, see spec 0002): a read only pass that reads the finished spec and returns a critique, writing nothing. It runs **automatically on a different model for `Full` and `Medium` tier features** (the tiers where these bugs actually live), is offered at `Lean`, and is skipped at `Vibe`; the main thread resolves every gap it finds before presenting. See *After the spec is written*.
